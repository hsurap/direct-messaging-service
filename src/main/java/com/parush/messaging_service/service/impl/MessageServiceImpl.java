...
public class MessageServiceImpl implements MessageService {
  ...
  public MessageResponse sendMessage(final SendMessageRequest request) {
    ...
    try {
      ...
      log.error("Business validation failed: {}", e.getMessage(), e);
      throw e;
    } catch (Exception e) {
      log.error("Error while sending message", e);
      throw new M2MException("Failed to send message");
    }
  ...
  public PagedResponse<MessageResponse> getConversationHistory(final String conversationId, final String requestingUserId, final String cursor, final int limit) {
    ...
    try {
      ...
      if (!conversation.getParticipantIds().contains(requestingUserId)) {
        log.error("User {} attempted to access conversation {} they are not part of", requestingUserId, conversationId);
        throw new M2MException("You are not authorized to view this conversation", "FORBIDDEN");
      }

      final int safeLimit = Math.min(Math.max(limit, 1), 100); // if bigger number is passed so that why's to guard it

      // fetching 1 extra to get to know is there any pending left or not
      final org.springframework.data.domain.Pageable pageable = org.springframework.data.domain.PageRequest.of(0, safeLimit + 1);
      ...
      if (cursor == null || cursor.isBlank()) {
        ...
        final Instant cursorInstant = CursorUtil.decode(cursor);
      ...
      final boolean hasMore = messages.size() > safeLimit;

      final List<Message> pageItems = hasMore ? messages.subList(0, safeLimit) : messages;
      final String nextCursor = hasMore
          ? CursorUtil.encode(pageItems.get(pageItems.size() - 1).getCreatedAt())
          : null;

      final List<MessageResponse> responses = pageItems.stream()
      ...
      log.error("Error fetching conversation history for conversationId: {}", conversationId, e);
      throw new M2MException("Failed to fetch conversation history");
    }
